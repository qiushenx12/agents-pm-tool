...
- 创建任务时提交来源规则不变（Web = 用户，Agent = Agent）；`owner_user_id` 记录实际创建账号，展示时用户任务使用用户名，Agent 任务使用 `Agent（用户名）`，不与现有 `submitter` 枚举及筛选冲突。
...
- [x] **T13 Agent 只读附件**（2026-09-10，任务 202609101047530000）：新增按 token 用户项目可见性校验的附件列表与下载接口；`pm-cli attachments/download` 支持列出、下载且默认不覆盖已有文件，不开放上传或删除能力。验收：集成测试覆盖元数据脱敏、文件下载、越权拒绝与只读边界。
- [x] **T14 提交人名称展示**（2026-09-10，任务 202609101104250000）：v9 迁移将未归属的历史任务回填给主机，新任务记录网页或 Agent 的实际创建账号，任务 DTO 提供 `submitter_name`；用户任务显示用户名，Agent 任务显示 `Agent（用户名）`，底层 `submitter` 来源枚举保持不变。验收：覆盖迁移、主机、普通用户、Agent 及同用户 Agent 不得修改网页任务的权限回归。
