...
| 完成时间 | 状态**每次进入** `待验证` 或 `已完成` 时刷新为当前时间（latest-wins，如验收未通过后重做再回到待验证，取最新一次）；离开这两个状态**不清空**；任何接口不可直接写入 |
...
- **单表网格**：9 列固定；行内编辑（项目/类型/状态下拉、描述点击进编辑）；列宽拖拽、按列排序（单字段升降序）；行高自适应描述换行。
- **筛选栏**：项目/类型/状态/提交人多选筛选 + 关键字（匹配 ID 与描述）；筛选条件进 URL query，刷新不丢。
- **新建/详情**：顶部「新建任务」弹窗（项目*、类型*、描述、附件）；点击行展开右侧抽屉，含完整描述、附件管理、时间信息。
- **删除任务**：仅网页端，行菜单内二次确认。
