...
- 任务的 `owner_user_id` 记录网页或 Agent 创建任务的所属用户。Agent 默认只能修改自己通过 Agent 创建的任务描述；有权管理该账号的管理员显式授予 `description_any_task` 后可修改可见项目中其他任务的描述，仍需账号字段权限；v8 迁移会把旧版全局 Agent 创建的存量任务归属到主机账号。
...
`agent_permission_profiles` 按用户配置 Agent 写权限（缺失行使用旧版默认配置）。默认可创建任务、修改状态为 `进行中`/`待验证`/`已完成`、修改优先级、修改当前 token 用户的 Agent 创建任务描述；创建时项目/类型/非空描述仍必填。管理员可调整普通用户 Agent 权限，超级管理员还可调整管理员及主机账号；可授权创建时的额外字段、可修改字段、状态值与描述修改范围。`GET /api/agent/permissions` 返回 token 的有效能力，`PATCH /api/agent/tasks/{id}` 与旧的单字段路由共用同一权限检查。

写权限始终与 token 所属用户的 `user_permissions` 取交集。主机/管理员可绕过 Web 普通用户字段授权，但不能绕过 Agent 配置。关联任务 ID 还要检查目标项目可见性，隐藏的既有关系不得被悄悄清除。

Agent 不可以：

- 直接指定或修改 ID、提交人、系统时间等不可变字段；
- 删除任务；
- 管理项目，或上传、删除附件；
- 直接读取/写入 SQLite。
