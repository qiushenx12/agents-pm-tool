...
- **任务工作台**：提供项目、类型、描述、备注、状态、提交人、附件和时间等固定字段，支持行内编辑与详情抽屉；备注列固定显示在最后并自动填满剩余宽度。
- **高效浏览**：支持多条件筛选、关键词搜索、分页、排序、分组，以及手动拖拽排序。
- **批量处理**：可批量修改项目、类型、状态，或批量删除任务。
- **视图偏好**：可调整列宽、列顺序与可见性，并将筛选、排序和分组保存为当前浏览器中的筛选方案。
...
# 创建任务：项目、类型、描述均为必填项
pm-cli create --project agents-pm-tool --type BUG --description "修复任务列表白屏"

# 查看与筛选任务
pm-cli list --status 进行中
pm-cli list --project agents-pm-tool --keyword "列表" --json
...
这些限制由 Rust 服务端强制执行，而不是只依赖 CLI 参数检查。验收相关状态保留给用户，Agent 不能自行将任务标记为“验收通过”或“验收未通过”。

## 任务规则

- 任务类型固定为：`新增需求`、`优化`、`BUG`。
- 状态固定为：`未开始`、`进行中`、`待验证`、`已完成`、`验收未通过`、`验收通过`。
