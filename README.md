...
- **任务工作台**：提供项目、类型、描述、备注、状态、前置任务 ID、解锁任务 ID、提交人、附件和时间等固定字段，支持行内编辑与详情抽屉；两个任务关系字段均可多选，单元格用英文逗号分隔 ID，并保持“前置 → 解锁”的双向关系一致。前置任务全部进入「已完成」或「验收通过」后，本任务才能切换到「进行中」。单击任务描述即可立即编辑，点击输入框外自动保存；列表底部可一键添加空白任务，顶部“+ 新建任务”仍提供完整新建弹窗，并会按浏览器记住上次在弹窗中选择的项目；备注列可像普通字段一样拖动排序和调整列宽，最右侧固定操作列可复制完整 Agent Prompt，ID 字段悬停按钮可复制一句话任务指引。侧栏顶部是“Agents PM Tool”模块：点击标题文字即可展开或收起项目列表（偏好随浏览器保存），标题右侧的加号用于新建项目，“Agents PM Tool”左侧的按钮可展开或收起整条侧栏。
...
- **视图偏好与关联图**：可调整列宽、列顺序与可见性，行高偏好随浏览器保存；分组、排序和筛选按账号保存在服务端，浏览器和「进入应用」窗口打开时都能恢复（带参数的 URL 仍可分享）。任务表页签的展开列表可保存当前浏览器中的筛选方案；旁边的「关联图」页签按当前项目范围展示任务依赖，默认隐藏无关联任务（可切换显示）。图中上游任务在左、下游任务在右，各列上下居中，以无箭头曲线连接；节点用状态色框区分，双击或按 Enter 打开同一任务详情。
...
pm-cli create --project default-project --type 优化 --description "实现新功能" --predecessor-task-ids "前置任务ID1,前置任务ID2"
...
| 修改前置/解锁任务 | ✓ | 需对应字段权限 | 需 Agent 对应字段权限，且关联任务须可见 |
...
- 前置任务与解锁任务是同一关系的两个方向，禁止自依赖和循环依赖；删除任务时相关关系自动清理。`待验证`不算完成，只有`已完成`或`验收通过`满足后续任务的开始条件。
